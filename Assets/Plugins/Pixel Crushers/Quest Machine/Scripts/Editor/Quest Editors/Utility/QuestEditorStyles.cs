        ...
        public static GUIStyle questNodeWindowGUIStyle
        {
            get
            {
                if (m_nodeWindowOnGuiStyle == null)
                {
                    ...
                    var textureName = "Quest Machine/" + (EditorGUIUtility.isProSkin ? "windowOnDark.png" : "windowOnLight.png");
                    var texture = EditorGUIUtility.Load(textureName) as Texture2D;
                    if (texture != null)
                    {
                        m_nodeWindowOnGuiStyle.onNormal.background = texture;
                    }
                }
