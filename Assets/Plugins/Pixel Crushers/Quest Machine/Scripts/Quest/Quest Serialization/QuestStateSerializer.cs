...
namespace PixelCrushers.QuestMachine
{
    ...
    public static class QuestStateSerializer
    {
        ...
        public static void DeserializeInto(Quest quest, byte[] bytes)
        {
        ...
        private static void WriteQuestDataToStream(BinaryWriter binaryWriter, Quest quest)
        {
            ...
            if (state == QuestState.WaitingToStart) return;
        ...
        private static void ReadTagDictionaryFromStream(BinaryReader binaryReader, TagDictionary tags, string questOrNodeName)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    if (Debug.isDebugBuild) Debug.LogWarning("Quest Machine: While deserializing quest tags, found a tag with a blank name in " + questOrNodeName + ".");
                }
                else if (tags.dict.ContainsKey(key))
                {
                    if (Debug.isDebugBuild) Debug.LogWarning("Quest Machine: While deserializing quest tags, found two tags with the name '" + key + "' in " + questOrNodeName + ".");
                }
                else
                {
