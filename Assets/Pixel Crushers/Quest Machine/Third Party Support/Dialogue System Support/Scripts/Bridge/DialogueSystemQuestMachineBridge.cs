...
namespace PixelCrushers.QuestMachine.DialogueSystemSupport
{
    ...
    public class DialogueSystemQuestMachineBridge : MonoBehaviour
    {
        ...
        {
            var qlc = QuestMachine.GetQuestListContainer(questGiverID);
        ...
        {
            var qlc = QuestMachine.GetQuestListContainer(questerID);
            if (qlc == null)
            {
        ...
        {
            var questInstance = QuestMachine.GetQuestInstance(questID, questerID);
            if (questInstance == null)
            {
                if (DialogueDebug.LogWarnings) Debug.LogWarning("Dialogue System: Can't find Quest Machine quest with ID '" + questID + "' to get its state.", this);
        ...
        private string GetQuesterQuestNodeNumberState(string questID, double nodeNumber, string questerID)
        {
            ...
            if (questInstance == null)
            {
                if (DialogueDebug.LogWarnings) Debug.LogWarning("Dialogue System: Can't find Quest Machine quest with ID '" + questID + "' to get its node " + entryNumber + " state.", this);
                return QuestLog.StateToString(DialogueSystem.QuestState.Unassigned);
            }
        ...
        private string GetQuesterQuestNodeState(string questID, string questNodeID, string questerID)
        {
            ...
            if (questInstance == null)
            {
                if (DialogueDebug.LogWarnings) Debug.LogWarning("Dialogue System: Can't find Quest Machine quest with ID '" + questID + "' to get its node '" + questNodeID + "' state.", this);
                return QuestLog.StateToString(DialogueSystem.QuestState.Unassigned);
            }
        ...
        private void SetCurrentQuestID(DialogueEntry entry)
        {
            ...
            if (conversation != null)
            {
                ...
                if (!string.IsNullOrEmpty(questID))
                {
                    ...
                    try
                    {
                        m_currentQuest = QuestMachine.GetQuestInstance(questID, questerID) ?? QuestMachine.GetQuestInstance(questID);
                    }
                    finally
                    {
